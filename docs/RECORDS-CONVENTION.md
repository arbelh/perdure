...
**Changing an earlier decision.** A decision that changes a decision recorded in an earlier record names that record's date-slug on its line and says why; if the earlier decision is an ADR, supersede it. Two records that disagree and name neither are unresolved: a reader holds both and does not pick by tone. The lint does not detect an alteration nobody named; a contradiction stated only in prose is not read. Decisions that other tasks must obey without ever reading this record belong in an ADR or the project's instruction file, not only here (see *Promotion at close*).

**Correcting a record** (learned fixing real rot, 2026-08-25): status headers are mutable state — correct them in place, with a short dated note saying what was wrong. A correction keeps a terminal record terminal; moving a record TO a terminal status is a close and runs through the closer. Body corrections get dated annotations rather than rewrites. **Paraphrase the retracted claim; never re-quote it verbatim** — quoted stale text reads as a current claim to anyone scanning the record and, worse, is exactly what misleads a derived-view or skimming reader. Scope surviving negative claims in time ("as of <date>, nothing had been committed") so they stay true forever instead of rotting.
...
- **Close** via `/perdure:close`, which runs `scripts/close-workflow.py` — the closer, in every mode: it refuses on an empty `# Review` (or records a conscious skip with `--skip-review "<reason>"`), lints, sets the canonical terminal status, stamps `updated:`, appends the abandon reason or outcome, and creates the HANDOFF page at a project's first close or regenerates it after. On an already-terminal record it verifies and refreshes the page. Before running it: reconcile any lint contradiction — a record must not go terminal while its status lies; confirm `# Review` carries a verdict or a recorded skip; name any earlier record whose decision this one changes; apply the promotion pass; fill `# Outcome`. A terminal status set by editing the file is a hand-close: it skips the stamp, the outcome line and the page, and the close-gate hook names it and points to the closer, which verifies an already-terminal record and creates or refreshes the page. The one allowed hand-close is a sandboxed child that cannot reach the closer; it says so in `# Outcome`, and the host runs the closer after. The hook re-lints automatically on any later edit to the terminal record.
- **Promotion at close**, per the `auto-promote-decisions` preference: `none` (default — list the workflow's decisions, ask which to promote), `tagged` (promote decisions marked `{promote}` inline), `heuristic` (apply the test below), `all` (promote everything). The test is inheritance, not size: promote when a task that never reads this record must still obey the decision — a default, a name, a boundary, a rule — and it can be stated in one sentence without this task's filenames. A one-line default other tasks inherit qualifies; a large refactor's local choices do not. Two homes: an ADR (`/perdure:decide <slug>` with context from the workflow file, then mark the decision `[promoted → #NNN]`) when the why and the alternatives matter; or a proposed one-line addition to the project's instruction file (`CLAUDE.md`, `AGENTS.md`) when only the rule matters — proposed, never written, per the write boundary. Before architecture-shaped work, check existing ADRs (`/perdure:decisions`) so the work doesn't silently contradict one.
